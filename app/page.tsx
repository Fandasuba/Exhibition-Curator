...
export default function Home() {
  ...
  useEffect(() => {
    if (showLoginModal && loginModalRef.current) {
      const firstInput = loginModalRef.current.querySelector('input');
      firstInput?.focus();
    }
  }, [showLoginModal]);
  ...
  useEffect(() => {
    if (showCreateModal && createModalRef.current) {
      const firstInput = createModalRef.current.querySelector('input');
      firstInput?.focus();
    }
  }, [showCreateModal]);

  useEffect(() => {
    if (showExhibitionModal && exhibitionModalRef.current) {
      const input = exhibitionModalRef.current.querySelector('input');
      input?.focus();
    }
  }, [showExhibitionModal]);
  ...
  useEffect(() => {
    if (exhibitionMessage && errorMessageRef.current) {
      errorMessageRef.current.setAttribute('aria-live', 'polite');
    }
  }, [exhibitionMessage]);
  ...
  // Exhibition click handler with improved accessibility
  ...
  // Enhanced keyboard support for exhibition cards
  const handleExhibitionKeyDown = (e: React.KeyboardEvent, exhibitionId: string) => {
    if (e.key === 'Enter' || e.key === ' ') {
      e.preventDefault();
      handleExhibitionClick(exhibitionId);
    }
  };
  ...
  const patchSavedItem = async (exhibitionId: string, itemIndex: number): Promise<void> => {
    ...
    try {
      ...
      setExhibitionMessage(`Failed to remove item: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
  ...
  const deleteExhibit = async (id: string) => {
    ...
    try {
      ...
      setExhibitionMessage(`Failed to delete exhibition: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setExhibitUpdate(false);
    }
  };
  ...
  if (loading) {
    return (
      <main className="p-8 max-w-4xl mx-auto" role="main" aria-label="Loading application">
        <div aria-live="polite" aria-label="Application is loading">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-amber-700 mx-auto mb-4" aria-hidden="true"></div>
          <p className="text-center text-amber-800">Loading Exhibition Curator...</p>
        </div>
      </main>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-amber-50 text-amber-900">
      ...
      >
        <header>
          <h1 className="text-3xl mb-6 text-amber-800 font-bold drop-shadow-sm">
        ...
        {isLoggedIn && (
          <section className="mb-6 p-4 bg-white/50 backdrop-blur-sm border border-amber-300 rounded-lg flex justify-between items-center" aria-label="User welcome section">
        ...
        {/* Auth Section */}
        {!isLoggedIn && (
          <section className="mb-6 p-4 bg-white/50 backdrop-blur-sm border border-amber-300 rounded-lg" aria-label="Authentication section">
        ...
        {isLoggedIn && (
          <>
            <section className="mb-8 p-6 bg-white/40 backdrop-blur-sm border-2 border-amber-300 rounded-lg" aria-label="Your exhibitions">
              ...
              {exhibitionsLoading ? (
                <div className="flex justify-center items-center py-12" aria-live="polite" aria-label="Loading exhibitions">
                      ...
                      return (
                        ...
                        >
                          <div 
                            ...
                            onKeyDown={(e) => handleExhibitionKeyDown(e, exhibition.id)}
                            tabIndex={0}
                            role="button"
                            aria-expanded={isExpanded}
                            aria-controls={`exhibition-content-${exhibition.id}`}
                            ...
                            className="flex flex-col sm:flex-row sm:h-32 focus:outline-none"
                          >
                            {/* Image Section */}
                            <div className="w-full h-48 sm:w-48 sm:h-full flex-shrink-0 relative overflow-hidden" role="img" aria-label={previewImage ? `Preview image for ${exhibition.name}` : `No preview image available for ${exhibition.name}`}>
                              {previewImage ? (
                                <img
                                  src={previewImage}
                                  alt={`Preview for ${exhibition.name} exhibition`}
                            ...
                            <div className="flex-1 p-4 flex justify-between items-center">
                              <div className="flex-1">
                                ...
                                <p className="text-sm text-amber-700">
                                  ...
                                  Click to <span className="text-amber-800">{isExpanded ? 'hide' : 'view'}</span>
                                </p>
                              </div>
                  ...
                  return (
                    <>
                      ...
                      {itemsLoading[selectedExhibition] ? (
                        <div className="flex justify-center items-center py-12" aria-live="polite" aria-label="Loading exhibition items">
                          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-amber-700" aria-hidden="true"></div>
                          <span className="ml-3 text-amber-800">Loading items...</span>
                        </div>
                      ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-6" role="grid" aria-label="Exhibition items grid">
              ...
              <form onSubmit={(e) => { e.preventDefault(); handleLogin(); }}>
                <div className="mb-4">
                  ...
                  <input
                    ...
                    className="border border-amber-400 rounded p-3 w-full focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 bg-white/80 text-amber-900 placeholder-amber-600 transition-colors"
                    required
              ...
              <form onSubmit={(e) => { e.preventDefault(); handleAddExhibition(); }}>
                <div className="mb-6">
                  ...
                  <input
                    ...
                    className="border border-amber-400 p-3 w-full rounded focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 bg-white/80 text-amber-900 placeholder-amber-600 transition-colors"
                    required
